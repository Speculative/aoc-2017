p = [[3093, 749, 3469, 142, 2049, 3537, 1596, 3035, 2424, 3982, 3290, 125, 249, 131, 118, 3138],
...
def solve(row_value):
    print(sum(map(row_value, p)))

# Part 1: sum of greatest difference per row
solve(lambda row: max(row) - min(row))

# Part 2: sum of divisible pairs
n = len(p[0])

solve(
    lambda row:
        sum(map(
            # Only catch this pair if the first member of the tuple is the larger one
            lambda pair: pair[0] // pair[1] if pair[0] % pair[1] == 0 else 0,
            # Pair every value with every value that isn't itself
            [(row[i], row[j]) for i in range(n) for j in range(n) if i != j]))
    )
